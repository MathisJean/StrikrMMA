...
const sections = Array.from(document.querySelectorAll('#record-section, #video-section, #timeline-section'));
...
sections.forEach(section => {
    ...
    const prompt_text = document.querySelector('#video-section .prompt')
    let slides = Array.from(section.querySelectorAll('.slideshow div'));
    let slide_index = null;

    slides.forEach(slide => {
        slide.addEventListener("click", (event) => {
            go_to_slide(slideshow, event.currentTarget);
        });
    });
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        if(slides.length == 0){
            prompt_text.style.display = "block";
        }
        else{
            go_to_slide(slideshow, slides[slides.length <= 1 ? 0 : 1])
        }
    });

    function go_to_slide(slideshow, element){
        ...
        let prev_drop_area = prev_slide.querySelector('.drop_area');
        let next_drop_area = next_slide.querySelector('.drop_area');
        let current_drop_area = current_slide.querySelector('.drop_area');
