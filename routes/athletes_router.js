...
router.get("/:username", async (req, res) => {
	...
	const events = await pool.query("SELECT * FROM events WHERE user_id = $1 ORDER BY order_index ASC", [profile.id]);
	const highlights = await pool.query("SELECT * FROM highlights WHERE user_id = $1 ORDER BY order_index ASC", [profile.id]);
